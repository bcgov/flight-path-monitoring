...
# Process one flight
flight_process.flightGPX <- function(flight, zones, telemetry, dist, max.altitude = units::set_units(500, m)) {
  ...
  # Digital elevation model
  dem <- bcmaps::cded(aoi, check_tiles = FALSE) |>
    terra::rast()
  
  # Transfrom flight points to linestring
  
  ground_level <- terra::extract(
    x = dem, 
    y = flight[["track_points"]] |> sf::st_geometry() |> sf::st_transform(crs = sf::st_crs(dem)) |> terra::vect(),
    method = "bilinear",
    ID = FALSE
  )[,1]
  
  # This is where we might want to do some smoothing is flight ele is bad
  above_ground_level <- pmax(flight[["track_points"]][["ele"]] - ground_level, 0)
  
  # Compute time deltas
  time_deltas <- difftime(
    flight[["track_points"]][["time"]][-1],
    flight[["track_points"]][["time"]][-nrow(flight[["track_points"]])],
    units = "secs"
  )
  
  ?sf::st_linestring
  flight[["track_points"]] |> sf::st_geometry() |> sf::st_coordinates() |> sf::st_linestring(dim = "XY") |> sf::st_as_sfc()
  # Cut above a differential elevation of 500m
  # Compute length of segment in incursion zones vs original length
